    ...
    for (index = 0; index < queue.length; index++) {
      ...
      if ("development" !== 'production' && has[id] != null) {
        ...
        if (circular[id] > config._maxUpdateCount) {
          warn('You may have an infinite update loop ' + (watcher === watcher.vm && watcher.vm._watcher ? 'in a component render function.' : 'in watcher with expression "' + watcher.expression + '"'), watcher.vm);
          break;
        }
      }
    }
